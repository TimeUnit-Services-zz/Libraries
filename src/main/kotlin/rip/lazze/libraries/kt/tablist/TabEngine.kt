...
class TabEngine {
    ...
    fun load() {
        ...
        TabThread().start()
        Library.getInstance().server.pluginManager.registerEvents(TabListeners(), Library.getInstance())
    }
    ...
    private fun fetchSkin(): PropertyMap? {
        val propertyMap = Library.instance.runBackboneRedisCommand { redis ->
        ...
        Library.getInstance().runBackboneRedisCommand { redis ->
            ...
            redis.setex(
                ...
                3600,
                getPropertyMapSerializer().serialize(localPropertyMap, null, null).toString()
            )
        }
