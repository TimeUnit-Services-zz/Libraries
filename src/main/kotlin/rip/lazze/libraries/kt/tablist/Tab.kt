...
class Tab(private val player: Player) {
    ...
    fun update() {
        if (Library.getInstance().tabEngine.layoutProvider != null) {
            val tabLayout = Library.getInstance().tabEngine.layoutProvider?.provide(this.player)
            if (tabLayout == null) {
                reset()
                return
            }
            ...
            for (y in 0 until TabLayout.HEIGHT) {
                for (x in 0 until TabLayout.WIDTH) {
                    ...
                    if (this.previousNames.containsKey(entryName)) {
                        ...
                        } else if (this.previousPings.containsKey(entryName) && this.pingToBars(this.previousPings[entryName]!!) != this.pingToBars(
                                ping
                            )
                        ) {
