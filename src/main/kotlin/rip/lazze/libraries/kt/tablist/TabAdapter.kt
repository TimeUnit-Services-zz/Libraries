...
class TabAdapter :
    PacketAdapter(Library.getInstance(), PacketType.Play.Server.PLAYER_INFO, PacketType.Play.Server.NAMED_ENTITY_SPAWN) {

    override fun onPacketSending(event: PacketEvent) {
        if (Library.getInstance().tabEngine.layoutProvider == null || !this.shouldForbid(event.player)) {
            return
        }
        ...
        if (event.packetType === PacketType.Play.Server.PLAYER_INFO) {
            ...
            Bukkit.getScheduler().runTask(Library.getInstance()) {
    ...
    private fun shouldCancel(player: Player, packetContainer: PacketContainer): Boolean {
        ...
        val trackerEntry =
            ((bukkitPlayer as CraftPlayer).handle.getWorld() as WorldServer).getTracker().trackedEntities.get(
                bukkitPlayer.entityId
            ) as EntityTrackerEntry
        return !trackerEntry.trackedPlayers.contains(recipient)
    }

    private fun shouldForbid(player: Player): Boolean {
        val playerName = player.name
        val playerTab = Library.getInstance().tabEngine.tabs[playerName]
